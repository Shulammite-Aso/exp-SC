...
import { cartSelector } from 'app/pages/CartPage/slice/selectors';
import { localStorageCartSelector } from 'app/pages/CartPage/slice/selectors';
...
export const TopbarCart = memo((props: Props) => {
  ...
  useEffect(() => {
    if (isUserLoggedIn) {
      ...
      setCartCount(productsInLocalStorageCart?.length);
    }
  }, [isUserLoggedIn, productsInCart, productsInLocalStorageCart]);
  ...
  return (
    <Wrapper>
      <TopbarCartMenu to="/cart" onClick={toggleProfile}>
        <CartCount>
          <p>{cartCount}</p>
        </CartCount>
        <CartIcon />
...
const TopbarCartMenu = styled(Link)`
